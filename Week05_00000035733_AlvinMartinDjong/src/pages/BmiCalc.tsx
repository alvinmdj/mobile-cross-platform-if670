...
const BmiCalc: React.FC = () => {
  ...
  // set BMI category & category color
  const bmiCategory = (bmi: number): CategoryProps => {
    if (bmi < 18.5) return {category: "Kurus", color: "warning"};
    else if (bmi <= 24.9) return {category: "Normal", color: "success"};
    else if (bmi <= 29.9) return {category: "Gemuk", color: "warning"};
    else return {category: "Obesitas", color: "danger"};
  }
  ...
  const calculateBMI = () => {
    ...
    // console.log(bmi);
    setCategory(bmiCategory(bmi).category);
    setCategoryColor(bmiCategory(bmi).color);
            ...
            <IonRow>
              <IonCol sizeSm='8' offsetSm='2' sizeMd='6' offsetMd='3'>
                <IonGrid>
                  ...
                  {calculatedBMI && <BmiResult calculatedBMI={calculatedBMI} category={category} categoryColor={categoryColor} />}
                </IonGrid>
              </IonCol>
            </IonRow>
          </IonGrid>
